...
class AuthRepository @Inject constructor(private val authApi: AuthApi) {
    ...
    fun getCredential(userNameOrEmail: String, password: String): Single<AuthDto> {
        val credentialFormBody = FormBody.Builder()
                .add(GRANT_TYPE_HEADER, PASSWORD)
                .add(USER_NAME, userNameOrEmail)
                .add(PASSWORD, password)
        return authApi.getAuthToken(credentialFormBody.build())
                .map { res -> AuthDto(res) }
    }

    fun getRefreshedToken(refreshToken: String): Single<AuthDto> {
...
//                .add(CLIENT_ID_HEADER, AUTHORIZATION_CLIENT_APP)
        return authApi.getAuthToken(refreshTokenFormBody.build())
                .map { res -> AuthDto(res) }
    }
